# Taken from https://github.com/javdejong/japanese-examples and modified to fit my needs
from aqt import mw
from aqt.qt import *
        ...
        if expression in dictionary:
            ...
            for j in index:
                ...
                if dictionary == dictionaries[0]:
                    example = example + " ✔"
                example = example.replace(expression,'<FONT COLOR="#83AA5D">%s</FONT>' %expression)
                ...
                if match:
                    ...
                    example = example.replace(expression_bis,'<FONT COLOR="#83AA5D">%s</FONT>' %expression_bis)
                elif match_reading:
...
def find_examples_multiple(n, maxitems):
    if DST_FIELD_COMB not in n and DST_FIELD_ENG not in n and DST_FIELD_JAP not in n:
...
def add_examples_temporarily(fields, model, data, collection):
    ...
    try:
        examples = find_examples_multiple(fields, config["maxShow"])
    except NoExamplesFoundException:
        return fields
...
def add_examples_focus_lost(flag, n, fidx):
...
if config["lookupOnAdd"]:
    addHook('editFocusLost', add_examples_focus_lost)
